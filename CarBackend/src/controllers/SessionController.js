...
async function login(req, res){
	...
	try {
		...
		*/
		...
		const token = jwt.sign({ id: user.id, validFor: expTime }, process.env.JWT_KEY, { algorithm: 'RS256', expiresIn: expTime })
		
		// Enviamos o token para o usuário, pois a autenticação foi feita com sucesso 
		return res.status(200).json({ token })
	} catch(err) { 
		return res.status(500).json({ error: "Erro interno do servidor ao validar credenciais" });
	}	
}
...
function verify(req, res){
	...
	*/
	...
	try{
		...
		const remainingMinutes = (new Date(decoded.exp * 1000) - new Date()) / (1000 * 60);
		...
		if(remainingMinutes > 0 && remainingMinutes < 10) {
			token = jwt.sign({ id: decoded.id, validFor: decoded.validFor }, process.env.JWT_KEY, { algorithm: 'RS256', expiresIn: decoded.validFor });
		}
